...
int main(int argc, char** argv) {
	...
	world_rank = (world_rank + 1) * pnum % world_size;
	/*
	 * TODO: end of additions
	 * - when added
	 */
	...
	if (world_rank != 0) {
		MPI_Recv(&token, 1, MPI_INT, recv_from, 0, MPI_COMM_WORLD,
				MPI_STATUS_IGNORE);
	...
	MPI_Send(&token, 1, MPI_INT, send_to, 0, MPI_COMM_WORLD);
